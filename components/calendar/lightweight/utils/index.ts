import { TimeDateFormat } from '../Calendar/Calendar.constants';
import { calculateStartAndEndMinute } from '../Calendar/Calendar.helper';
import {
...
export const prepareCalendarData = (
  ...
  for (const obj of sortedCalendarValue) {
    ...
    // Check if this is a late-night event (ends between 00:00 and 05:59 next day)
    const endHour = getHours(endDate);
    const endMinute = getMinutes(endDate);
    const endsAtMidnight = endDate.getTime() === endDateModified.getTime();
    const isLateNightEvent = !endsAtMidnight && endHour < 6 &&
      differenceInDays(endDateModified, startDateModified) === 1;

    // If event ends at midnight or is a late-night event, don't show on next day
    const numberOfDays: number = differenceInDays(
      endDateModified,
      startDateModified,
    ) - ((endsAtMidnight || isLateNightEvent) ? 1 : 0);
...
export const prepareCalendarDataWithTime = (
  ...
  sortedCalendarValue.forEach((obj) => {
    ...
    if (numberOfMinutes > 24 * 60) {
      ...
      // Check if event ends in early morning (> 00:00 and < 6 AM) of next day - treat as late-night extension
      // Events ending at exactly midnight (00:00) are not late-night events
      const endHour = getHours(endDate);
      const endMinute = getMinutes(endDate);
      const isLateNightEvent = (endHour > 0 || endMinute > 0) && endHour < 6;

      // First day
      let firstDayEndMinute: number;

      if (isLateNightEvent) {
        // For late-night events, extend past midnight (24:00, 25:00, etc.)
        const { startMinute } = calculateStartAndEndMinute(startDate, startDate);
        const extendedEndMinute = 24 * 60 + getHours(endDate) * 60 + getMinutes(endDate) + 1;
        firstDayEndMinute = extendedEndMinute;
        const firstDayRes = {
          ...res,
          startMinute,
          endMinute: firstDayEndMinute,
        };
        result.day[key] = [...(result.day[key] || []), firstDayRes];
        // Don't create second-day entry for late-night events - they're shown on first day only
      } else {
        // Normal multi-day event - cap at end of first day
        const firstDayEnd = endOfDay(startDate);
        const { startMinute, endMinute } = calculateStartAndEndMinute(
          startDate,
          firstDayEnd,
        );
        firstDayEndMinute = endMinute;
        const firstDayRes = {
          ...res,
          startMinute,
          endMinute: firstDayEndMinute,
        ...
        // Next day - only for non-late-night events
