...
import { MIN_HOUR } from './Calendar.constants';
...
const CalendarWrapper: React.FC<CalendarWrapperProps> = ({
  ...
  const renderDayWeekTimeItems = ({
    ...
    const hourOffset = (hourRange?.min ?? MIN_HOUR) * 60; // Offset in minutes from start of day to min hour

    return ((preparedData as PreparedDataWithTimeFull).day[key] || []).map(
      (preparedDataItem, index) => {
        // Adjust grid positions to account for hour offset
        const adjustedStart = Math.max(1, preparedDataItem.startMinute - hourOffset);
        const adjustedEnd = preparedDataItem.endMinute - hourOffset;

        return (
          <div
            key={`${index}-${dateInfo.date}`}
            style={{
              [!isDayReverseView ? 'gridRow' : 'gridColumn']: `${adjustedStart} / ${adjustedEnd}`,
